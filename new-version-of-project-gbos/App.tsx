...
import {
  INITIAL_DOSSIER, MEMORY_NODES as CORE_NODES, RELATIONSHIP_TREE, BRIEFING_SECTIONS,
...
const App: React.FC = () => {
  ...
  const handleStoryNodeComplete = useCallback((nodeId: number) => {
    setGameState(prev => {
      ...
      const filterByRegistry = (id: string) => {
        ...
        if (!meta) return true;
        // Years are always persistent by nature in this narrative
        if (meta.type === 'year') return true;
        if (meta.isIdentity || meta.isPersistent) return true;
        // Keep keywords from future chapters or the current one if not specifically expiring
        if (!meta.chapter || meta.chapter >= nodeId) return true;
        return false;
      };
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    Object.entries(GLOBAL_KEYWORD_MAP).forEach(([alias, meta]) => {
      if (lowerQuery.includes(alias.toLowerCase())) detectedKeywordIds.push((meta as any).id);
    });
      ...
      setTimeout(() => {
        setGameState(prev => {
          ...
          const filterConsumed = (id: string) => {
            const meta = KEYWORD_REGISTRY[id];
            if (meta?.isPersistent || meta?.type === 'year') return true;
