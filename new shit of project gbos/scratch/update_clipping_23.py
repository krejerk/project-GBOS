...
# Updated content for Clipping 23 annotation
...
# Fix literal newlines in strings before parsing
...
for item in data:
    if item['id'] == 'clipping_23':
        ...
        # Update revealed list if not already updated
        if 'revealed' not in item:
            item['revealed'] = []
        if "MTXXXXXXXX-93" not in item['revealed']:
            item['revealed'].append("MTXXXXXXXX-93")
        if "QTC-VA-0994" not in item['revealed']:
            item['revealed'].append("QTC-VA-0994")
        break
...
print("Clipping 23 updated successfully in database.")
