...
import { CLUE_DISPLAY_MAP, KEYWORD_REGISTRY } from '../constants';
...
export const DialogueView: React.FC<DialogueViewProps> = ({
    ...
    const handleClueClick = (clueId: string) => {
        ...
        // Trigger flash
        setClickedClueId(clueId);
        setTimeout(() => setClickedClueId(null), 800);
    };
                    ...
                    <motion.span
                        ...
                        className={`
                            ...
                            ${isCollected
                                ...
                                : 'text-[#c85a3f] hover:bg-[#c85a3f]/20 border-b border-dashed border-[#c85a3f]/50'
                            }
                        `}
                    >
                        {text}
                        {clickedClueId === clueId && (
                            <motion.span 
                                ...
                                className="absolute inset-0 bg-white rounded z-10"
                            />
                        )}
                    </motion.span>
                );
                                ...
                                >
                                    ...
                                    <div className={`relative max-w-[90%] md:max-w-[80%] ${isSystem ? 'w-full text-center py-4' :
                                        ...
                                        <p className={`text-xl md:text-2xl leading-relaxed ${isSystem ? 'text-[#c85a3f] text-sm italic tracking-widest' :
                                            ...
                                            {isLatest ? renderContent(displayedText) : renderContent(dialogueLine.text)}
                                            {isLatest && isTyping && (
                        ...
                        {showPasswordInput && (
                            ...
                            >
                                <div className="flex flex-col items-center gap-4">
                                    ...
                                    {(collectedClues.length > 0 || unlockedPeople.length > 0 || (collectedDossierIds && collectedDossierIds.length > 0)) && (
                                            ...
                                            {Array.from(new Set([...collectedClues, ...unlockedPeople, ...(collectedDossierIds || [])]))
                                                ...
                                                .map(id => (
                                                    <button
                                                        ...
                                                        className="px-3 py-1 bg-[#d89853]/5 hover:bg-[#d89853]/10 border border-[#d89853]/20 rounded-full text-[#d89853] text-xs transition-colors shrink-0"
                                                    >
