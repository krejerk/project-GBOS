...
export const SyndicateBoard: React.FC<SyndicateBoardProps> = ({
    ...
    // Relationship Tree is deprecated and disabled.
    const visibleNodes = [];
    ...
    return (
        <div className={`${isEmbedded ? 'relative w-full h-full overflow-hidden' : 'fixed inset-0 z-[60] bg-black overflow-y-scroll overflow-x-hidden custom-scrollbar'}`} style={!isEmbedded ? { scrollbarGutter: 'stable' } : {}}>
            ...
            <AnimatePresence>
                {(!isLightsOn) && (
                    <motion.div
                        ...
                        className={`${isEmbedded ? 'absolute inset-0' : 'absolute top-0 left-0 w-full min-h-screen'} z-[9000] bg-black flex flex-col ${isExiting ? 'pt-[110px]' : 'justify-center'} items-center cursor-pointer transition-all duration-1000`}
            ...
            <motion.div
                ...
                className={`relative ${isEmbedded ? 'h-full w-full' : 'min-h-[1600px] w-full'} flex flex-col items-center ${isEmbedded ? 'pt-16' : 'pt-32'} pb-32 z-10`}
            >
                ...
                <div
                    ref={containerRef}
                    className={`relative ${isEmbedded ? 'w-full scale-90' : 'w-full md:w-[1300px]'} min-h-[2500px] md:min-h-[1000px] bg-[#050505] z-10 shadow-[0_50px_120px_rgba(0,0,0,1)] border-[4px] border-[#222] rounded-sm overflow-y-auto md:overflow-hidden scrollbar-hide`}
                    style={{
                        backgroundImage: `
