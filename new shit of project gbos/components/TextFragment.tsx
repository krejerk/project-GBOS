...
interface TextFragmentProps {
    ...
    onKeywordClick: (e: React.MouseEvent, word: string, forceClueId?: string) => void;
...
export const TextFragment: React.FC<TextFragmentProps> = React.memo(({
    ...
    const contentParts = useMemo(() => {
        // Exclusion list for words that should NEVER be auto-highlighted globally in text blocks
        ...
        const keywords = Object.keys(keywordMap)
            .filter(name => !highlightBlacklist.includes(keywordMap[name]))
            .sort((a, b) => b.length - a.length);
        
        // Create regex that captures both manual [text](clue:id) tags AND automatic keywords
        ...
        if (!pattern) return [{ text, isKeyword: false }];
            ...
            <p className={`leading-loose text-lg text-justify tracking-wide opacity-90 ${useReporterStyle ? 'font-decayed' : 'font-serif text-[var(--confess-text)]'}`}>
                {(() => {
                    ...
                    return contentParts.map((part, j) => {
                        // A part is pickable if it's a keyword AND it's the first time it's been seen in the whole document (passed down via highlightKeywords)
                        // Manual tags force their own ID, automatic tags resolve via keywordMap
                        ...
                        if (isPickable) {
                            ...
                            return (
                                <span
                                    key={j}
                                    onClick={(e) => {
                                        // If manual, we bypass the name-to-id lookup and use the forced ID
                                        if (part.isManual && part.clueId) {
                                            e.stopPropagation();
                                            onKeywordClick(e, part.text, part.clueId);
                                        } else {
                                            onKeywordClick(e, part.text);
                                        }
                                    }}
                                    className={`
